...
export const saveColorData = async (
    lineData: IColor[]
) => {
...
export const getAllColor = async (): Promise<IColor[]> => {
    const db = await initDB();
...
export const checkIfColorStore = async () => {
    const db = await initDB();
    ...
    const allItems: IColor[] = await store.getAll();
...
export const clearColorData = async () => {
    const db = await initDB();
