...
export const getAllBuyer = async (): Promise<IBuyer[]> => {
    const db = await initDB();
...
export const checkIfBuyerStore = async () => {
    const db = await initDB();
    ...
    const allItems: IBuyer[] = await store.getAll();
...
export const clearBuyerData = async () => {
    const db = await initDB();
