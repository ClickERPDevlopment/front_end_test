...
const TnaDashboardMissingActualDateList = () => {
  const { missingActualDateTaskList, missingActualDateTaskLoading  } = useSelector((state: RootState) => state.tna);
  ...
  const rowRenderer = ({ index, key, style }: { index: number; key: string; style: React.CSSProperties }) => {
    const row = missingActualDateTaskList[index];
    return (
      <div
        key={key}
        style={style}
        className={`grid grid-cols-9 text-sm h-[30px]
          transition-colors 
      ...
      >
        ...
        <div className={cellClass}>{row.pono}</div>
        <div className={cellClass}>{row.taskName}</div>
        <div className={cellClass}>{row.prettyEffectDate}</div>
        <div className={cellClass}>{row.delayDate}</div>
  ...
  return (
    <div className="overflow-x-auto rounded-2xl shadow-lg border border-black-50">
      <div ref={containerRef} className="rounded-2xl border border-black-50">
        {/* Header */}
        <div className="grid grid-cols-9 bg-gradient-to-r from-sky-600 to-indigo-600 text-white text-sm uppercase sticky top-0 z-20 shadow-md">
         
          <div className={cellClass2}>O.P.M</div>
          <div className={cellClass2}>Job Number</div>
          <div className={cellClass2}>Style No.</div>
          <div className={cellClass2}>Buyer</div>
          <div className={cellClass2}>PO No.</div>
          <div className={cellClass2}>Task Name</div>
          <div className={cellClass2}>Due Date</div>
          <div className={cellClass2}>Delay Days</div>
          <div className={cellClass2}>Action</div>
        </div>

        {/* Virtualized List */}
        {!missingActualDateTaskLoading && width > 0 && (
          <div style={{ height: height+'px', overflow: 'hidden' }}>
            <List
              height={height}
              rowCount={missingActualDateTaskList.length}
              rowHeight={30}
        ...
        {
          missingActualDateTaskLoading && (
            <div className="grid grid-cols-9 h-[40px] place-items-center">
...
export default TnaDashboardMissingActualDateList;
