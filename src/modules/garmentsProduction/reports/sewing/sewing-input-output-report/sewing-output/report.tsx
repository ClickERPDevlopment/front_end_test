...
function SewingOutputReport({
  ...
  function groupBy(data: SewingOutputDetailsReportType[], keys: string[]) {
    return data.reduce((result: any, item: any) => {
      ...
      return result;
    }, {} as Record<string, { items: SewingOutputDetailsReportType[] }>);
  }
  ...
  const secondHeader = ["Output Qty", "Remarks"];
  ...
  const totalQty = data.reduce((acc, item) => acc + item.SEWINGOUTPUT, 0);
...
export default SewingOutputReport;
