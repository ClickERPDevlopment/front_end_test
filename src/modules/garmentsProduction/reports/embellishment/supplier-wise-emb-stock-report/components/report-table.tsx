...
function ReportTable({
  ...
  const pProcess = data?.reduce((acc, item) => acc + item.P_RECEIVEQTY, 0)
  const eProcess = data?.reduce((acc, item) => acc + item.E_RECEIVEQTY, 0)
  const peProcess = data?.reduce((acc, item) => acc + item.PE_RECEIVEQTY, 0)
  const wProcess = data?.reduce((acc, item) => acc + item.W_RECEIVEQTY, 0)
  ...
  return (
    <>
      <tr style={{ fontSize: "12px" }} className="font-bold">
        <td colSpan={18} className="border border-gray-950 p-0.5">{data[0]?.SUPPLIER}</td>
      </tr>
      ...
      <tr style={{ fontSize: "12px" }} className="font-bold">
        <td colSpan={4} className="border border-gray-950 p-0.5 text-center">Supplier Total</td>
