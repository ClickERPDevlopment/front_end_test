...
function ReportTable({
  ...
  const groupedData: Record<string, Record<string, DateWiseFinishingProoductionReportType>> = {};

  data.forEach((item) => {
    const dateKey = moment(item.FINISHINGDATE).format("YYYY-MM-DD");
    ...
    if (!groupedData[dateKey][companyKey]) {
      ...
      existing.FINISHINGQTY += item.FINISHINGQTY;
  ...
  sortedDates.forEach((date) => {
    companyHeader?.forEach((prefix) => {
      ...
      if (item) {
        ...
        companySummary[prefix].FINISHINGQTY += item.FINISHINGQTY;
        companySummary[prefix].WORKING_HOUR += item.WORKING_HOUR;
        companySummary[prefix].PER += ((item.FINISHINGQTY / item.TARGET) * 100) || 0;
  ...
  Object.values(companySummary).forEach((summary) => {
    ...
    grandTotal.FINISHINGQTY += summary.FINISHINGQTY;
    grandTotal.WORKING_HOUR += summary.WORKING_HOUR;

    avgTotal.TARGET += summary.TARGET / summary.COUNT;
    avgTotal.FINISHINGQTY += summary.FINISHINGQTY / summary.COUNT;
        ...
        <tbody>
            ...
            const tableRow = <React.Fragment key={date}>
              <tr key={date}>
                ...
                {companyHeader?.map((prefix) => {
                  ...
                  const deviation = item.FINISHINGQTY - item.TARGET;
                  const percent = item.TARGET > 0 ? ((item.FINISHINGQTY / item.TARGET) * 100) : 0;

                  totalTarget += item.TARGET;
                  totalCuttingQty += item.FINISHINGQTY;
                  totalWorkingHour += item.WORKING_HOUR;

                  grandTotalTarget += item.TARGET;
                  grandTotalCuttingQty += item.FINISHINGQTY;
                  grandTotalWorkingHour += item.WORKING_HOUR;

                  return (
                    <React.Fragment key={prefix}>
                      <td className="border border-gray-950 p-1 text-center">{Math.round(item.TARGET)}</td>
                      <td className="border border-gray-950 p-1 text-center">{Math.round(item.FINISHINGQTY)}</td>
          ...
          <tr className="bg-green-100 font-semibold" style={{ backgroundColor: "#fbffdd" }}>
            ...
            {companyHeader?.map((prefix) => {
              ...
              const cuttingQty = summary.FINISHINGQTY / summary.COUNT;
              const deviation = (summary.FINISHINGQTY - summary.TARGET) / summary.COUNT;
              const percent = (summary.FINISHINGQTY * 100 / summary.TARGET);
          ...
          <tr className="bg-green-100 font-semibold" style={{ backgroundColor: "#fbffdd" }}>
            ...
            {companyHeader?.map((prefix) => {
              ...
              const deviation = summary.FINISHINGQTY - summary.TARGET;

              return (
                <React.Fragment key={prefix}>
                  <td className="border border-gray-950 p-1 text-center">{Math.round(summary.TARGET)}</td>
                  <td className="border border-gray-950 p-1 text-center">{Math.round(summary.FINISHINGQTY)}</td>
                  <td className="border border-gray-950 p-1 text-center">{Math.round(deviation)}</td>
                </React.Fragment>
              );
            })}
            ...
            {companyHeader?.map((prefix) => {
              ...
              return (
                <tr key={prefix}>
                  ...
                  <td className="border border-gray-950 p-1 text-center">{Math.round(summary.TARGET)}</td>
                  <td className="border border-gray-950 p-1 text-center">{Math.round(summary.FINISHINGQTY)}</td>
                  <td className="border border-gray-950 p-1 text-center">{(summary.FINISHINGQTY * 100 / summary.TARGET).toFixed(2)} %</td>
