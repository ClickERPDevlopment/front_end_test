...
function KnittingProductionReport() {
  const [data, setData] = useState<KnittingProductionReportType[]>([]);
  ...
  const floorId: number = Number(searchParams.get("floorId")) || 0;
  const fromDate: string = searchParams.get("fromDate") || "01-Jun-25";
  const toDate: string = searchParams.get("toDate") || "10-Jun-25";
  const dtChecked: boolean = true //searchParams.get("dtChecked") === "True";
    ...
    async function getData() {
      ...
      try {
        const response = await axios.get(
          `${api.ProductionUrl}/production/KnittingReport/KnittingProductionReport`,
          {
            params: {
              id,
              factoryId,
...
export default KnittingProductionReport;
