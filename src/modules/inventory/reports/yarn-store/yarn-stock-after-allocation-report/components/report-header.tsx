/* eslint-disable @typescript-eslint/no-explicit-any */
import moment from "moment";
import { LotWiseYarnStockReportType } from "../yarn-stock-after-allocation-report-type";

function ReportHeader({
  searchParams
}: {
  data: LotWiseYarnStockReportType[];
  searchParams: { dtDate: string, chOnlyWhiteLot: boolean }
}) {

  return (
    <div className="w-[100%]">

      {/* <p className="font-bold text-left w-[100%] text-xs">
        {moment(searchParams.dtDate).format("DD-MMM-YYYY")}
      </p> */}

      {
        // data[0]?.MAIN_COMPANY && <>
        //   <h1 className="font-bold text-xl text-center">
        //     {
        //       data[0]?.MAIN_COMPANY
        //     }
        //   </h1>
        //   <h4 className="font-bold text-sm text-center mb-3">
        //     {
        //       data[0]?.MAIN_COMPANY_ADDRESS
        //     }
        //   </h4>
        // </>
      }

      {/* <h1 className="font-bold text-sm text-center">
        ({
          data[0]?.COMPANY_NAME
        })
      </h1> */}
      <h3 className="font-bold text-xl text-center">
        {searchParams.chOnlyWhiteLot && "(WHITE)"} Yarn Stock After Allocation Report
      </h3>
      <p className="font-bold text-center w-[100%] text-lg">
        Of {moment(searchParams.dtDate).format("DD-MMM-YYYY")}
      </p>
    </div>
  );
}

export default ReportHeader;
