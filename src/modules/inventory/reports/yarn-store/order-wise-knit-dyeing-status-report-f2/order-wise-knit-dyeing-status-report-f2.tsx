...
export default function OrderWiseKnittingDyeingStatusReportF2({ data }: { data: OrderWiseKnttingDeyingStatusReportTypeF2[] }) {

  const uniquejob = [...new Set(data?.map(item => item.PONO))]; // [ 'A', 'B']

  const getTotal = (fieldName: keyof OrderWiseKnttingDeyingStatusReportTypeF2, d: OrderWiseKnttingDeyingStatusReportTypeF2[]) =>
      ...
      <div className='table-wrapper'>
        <table className='table table-bordered'>
          ...
          <tbody>
              ...
              return (
                <>
                  ...
                  <tr className="border font-bold bg-emerald-200 text-base">
                    <td className="font-bold" colSpan={12}>PO Total</td>

                    <td className="font-bold">{(data?.filter(f => f.PONO === po))[0].PO_QTY}</td>
            ...
            <tr className="border font-bold bg-emerald-200 text-base">
              <td className="font-bold" colSpan={12}>Grand Total</td>

              <td className="font-bold"></td>
              <td>{getTotal("ORDER_QTY_FIN", data)}</td>
              <td></td>
              <td>{getTotal("REQ_YARN_QTY", data)}</td>
              <td>{getTotal("YARN_ISSUE_QTY", data)}</td>
              <td>{getTotal("YARN_ISSUE_BAL", data)}</td>
              <td>{getTotal("REQ_GREY_QTY", data)}</td>
              <td>{getTotal("KNT_QTY", data)}</td>
              <td>{getTotal("KNT_BAL", data)}</td>
              <td>{getTotal("GREY_RCV_QTY", data)}</td>
              <td>{getTotal("GREY_RCV_BAL", data)}</td>
              <td>{getTotal("GREY_ISSUE_QTY", data)}</td>
              <td>{getTotal("GREY_ISSUE_BAL", data)}</td>
              <td>{getTotal("DYEING_QTY", data)}</td>
              <td>{getTotal("DYEING_BAL", data)}</td>
              <td>{getTotal("FIN_QTY_GW", data)}</td>
              <td>{getTotal("FIN_QTY_FW", data)}</td>
              <td>{getTotal("FIN_FW_BAL", data)}</td>
              <td>{getTotal("FIN_GW_BAL", data)}</td>
              <td>{getTotal("WASTAGE", data)}</td>
              <td>{getTotal("FIN_RCV_QTY", data)}</td>
              <td>{getTotal("FIN_RCV_BAL", data)}</td>
              <td>{getTotal("ISSUE_TO_CUT", data)}</td>
              <td>{getTotal("ISSUE_TO_CUT_BAL", data)}</td>
            </tr>
          </tbody>
        </table>
      </div>
