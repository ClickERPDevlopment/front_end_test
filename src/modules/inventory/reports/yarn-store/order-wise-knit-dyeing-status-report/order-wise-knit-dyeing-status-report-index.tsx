...
import OrderWiseKnittingDyeingStatusReport from "./order-wise-knit-dyeing-status-report";

export default function OrderWiseKnittingDyeingStatusReportIndex() {
  const [data, setData] = useState<OrderWiseKnttingDeyingStatusReportType[]>([]);
  ...
  useEffect(() => {
    async function getData() {
      try {
        ...
        await axios
          .get(
            `${api.ProductionUrl}/production/YarnStoreReport/OrderWiseKnittingDyeingStatusReport?COMPANY_LIST=${COMPANY_LIST}&` +
  ...
  return isLoading ? (
    ...
    <>
      <div className="p-5">
        <OrderWiseKnittingDyeingStatusReport data={data} />
      </div>
    </>
  );
}
